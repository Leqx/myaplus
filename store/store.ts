...
const immer = (config: (arg0: (fn: any) => void, arg1: any, arg2: any) => any) => (set: (arg0: any) => void, get: any, api: any) =>
  config(
    (fn: any) => {
...
const zustandCreateStore = (children: (set: any, get: any) => { init: () => Promise<void>; updateTodo: (item: any) => Promise<unknown>; deleteTodo: () => void; updateSelectedTask: (item: any) => Promise<unknown>; deleteSelectedTask: (item: any) => Promise<unknown>; todo: never[]; }) => create(immer(children));

const storeStates = {
  todo: []
};

const storeMethods = (set: (arg0: { todo: any; }) => void, get: () => { (): any; new(): any; todo: any[]; }) => ({
  init: async () => {
    try {
  ...
  updateTodo: async (item: { date: any; todoList: any; }) =>
    new Promise<void>(async (resolve) => {
      ...
      if (datePresent) {
        const updatedTodo = get().todo.map((data: { date: any; todoList: any; }) => {
  ...
  deleteSelectedTask: async (item: { date: any; todo: { key: any; }; }) =>
    new Promise<void>(async (resolve) => {
      ...
      const checkForEmpty = newTodo.filter((data: { todoList: string | any[]; }) => {
